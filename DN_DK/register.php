...
// Kiểm tra xem tên đăng nhập và email có tồn tại 
if (empty($errors)) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM users WHERE FULLNAME = :ten");
        $stmt->bindParam(':ten', $ten);
        $stmt->execute();
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($user) {
            $errors['ten'] = "Tên đăng nhập đã tồn tại trong hệ thống.";
        }
        ...
        if (empty($errors)) {
            ...
            $query = $pdo->prepare("INSERT INTO users (FULLNAME, USERPASSWORD, PHONE, EMAIL, DAYOFBIRTH, USERADDRESS, PICTURE, ROLEID, province_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $query->execute([$ten, $hashed_password, $phone, $email, $birth, $province_name, $_FILES["hinhdaidien"]["name"], $role_id, $province_id]);
    ...
    <form action="register.php" method="post" enctype="multipart/form-data">
        <table align="center">
            ...
            <tr>
                ...
                <td><input type="text" id="phone" name="phone"><?php if (!empty($errors['phone'])) echo "<span style='color: red;'>{$errors['phone']}</span>"; ?></td>
            </tr>
